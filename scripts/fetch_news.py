...
# "종합"만 괄호도 인정한다(2026-09-24 사용자 요청) — 연합뉴스·뉴스1은 여러 소식을
# 묶은 기사 제목 끝에 대괄호 "[종합]" 대신 소괄호 "(종합)"을 붙이는 관행도 쓴다(예:
# "…한국행…별도 정착 지원 예상(종합)"). "종합" 앞뒤에 공백만 허용하고 다른 글자는
# 안 되므로("(종합 순위 발표)"처럼 단어가 더 있으면 매치 안 됨) 오탐 위험은 낮다.
# 속보/단독은 괄호 표기 관행이 확인된 바 없어 그대로 둔다.
TAG_RE = {
